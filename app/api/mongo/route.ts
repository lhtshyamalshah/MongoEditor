...
function verifyRequest(request: NextRequest) {
  ...
  if (!/^(localhost|127\.0\.0\.1|\[::1\])(?::\d+)?$/.test(host))
    throw new AppError(
      "This database browser only accepts local requests.",
      403,
    );
  ...
  if (
    request.method === "POST" &&
    (request.headers.get("x-mongo-browser") !== "1" ||
...
export async function GET(request: NextRequest) {
  try {
    ...
    return respond({ profiles: profileNames() });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (body.length > 2_100_000)
        throw new AppError("Request is too large.", 413);
      input = object(JSON.parse(body), "Request");
    } catch (error) {
    ...
    if (input.action === "collections")
      return respond(await collections(session, input));
