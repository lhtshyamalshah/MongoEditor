...
export default function PostgresTableList({
  items,
  selected,
  search,
  onSelect,
  ...
  const tables = items.map((item) => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) pair = parsed;
    } catch {
      /* Use the display label until metadata refreshes. */
    }
    return {
      ...item,
      schema: item.schema || pair[0] || "Unknown schema",
      table: item.table || pair[1] || item.label || item.name,
  ...
  function tableButtons(rows: typeof tables) {
    return rows.map((item) => (
      <button
        ...
        className={`collection-item ${selected === item.name ? "active" : ""}`}
      ...
      >
        ...
        {item.readOnly || item.type === "view" ? (
          ...
          selected === item.name && <ChevronRight size={13} />
        )}
      </button>
    ));
  }
  function group(
    ...
    return (
      <section className={`table-group${nested ? " nested" : ""}`} key={key}>
        <button
          type="button"
          className="table-group-heading"
          aria-expanded={open}
          onClick={() => toggle(key)}
  ...
  return (
    <>
      <label className="table-group-control">
        Group by
        <select
          aria-label="Group PostgreSQL tables by"
          value={groupBy}
          onChange={(event) => {
            setGroupBy(event.target.value as "owner" | "schema");
      ...
      {groupTables(tables, groupBy).map(([label, rows]) =>
        group(
          ...
          groupBy === "schema"
            ? tableButtons(rows)
            : groupTables(rows, "schema").map(([schema, schemaRows]) =>
                group(
                  JSON.stringify(["owner", label, schema]),
                  schema,
                  schemaRows.length,
                  true,
                  tableButtons(schemaRows),
                ),
              ),
        ),
      )}
