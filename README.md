...
- **New query** opens another tab, up to 25. Each tab keeps its own draft, parameter values, row limit, and results, so you can compare queries without losing work. Tabs are named after the first `FROM`/`JOIN` table in their query, falling back to `Query 1`, `Query 2`, and so on. Closing an edited tab asks for confirmation. Tabs live in memory only: changing database/connection or reloading the page resets them to a single tab.
- One query runs per connection at a time, so **Run query** is disabled in the other tabs while a query is running. A running tab shows a spinner in its tab name, and you can keep editing other tabs while it finishes.
...
- Results support table/JSON views and copying. **Copy CSV** copies every returned row, with a header line, RFC 4180 quoting, and CRLF line endings; SQL NULL becomes an empty field, so use **Copy JSON** when you need to tell NULL from an empty string. Values remain PostgreSQL text to preserve numeric/timestamp precision; SQL NULL remains `null` in JSON. JSON uses separate column names and row arrays so duplicate result-column names are preserved. Results are read-only: they are never edited or deleted through the SQL console.
