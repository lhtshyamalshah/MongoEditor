...
function fixture(keyed = true) {
  ...
  const pool = {
    query: async (text: string, values: unknown[] = []) => {
      ...
      return {
        rows: Array.from({ length: keyed ? 11 : 10 }, (_, index) => ({
          id: String(index + 1),
          tenant: "a",
        })),
      };
  ...
  const session = {
    ...
    database: "test",
    touched: Date.now(),
    pools: new Map([["test", pool]]),
  } as unknown as ReturnType<typeof sessionFor>;
  const input = {
    database: "test",
    collection: '["public","items"]',
    pageSize: 10,
  };
  return {
    session,
...
test("opening both panels shares one catalog read; each page performs one bounded query and no count", async () => {
  ...
  const [first] = await Promise.all([
    documents(session, input),
    schema(session, input),
    schema(session, input),
  ]);
  const second = await documents(session, {
    ...input,
    page: 2,
  ...
  assert.equal(
    queries.filter((query) => query.text.includes("pg_attribute")).length,
    1,
  );
...
test("metadata refresh is explicit, expires after a minute, and shares concurrent refreshes", async (context) => {
  ...
  await Promise.all([
    schema(session, { ...input, refresh: true }),
    schema(session, { ...input, refresh: true }),
  ]);
...
test("table discovery is cached; failed metadata requests remain retryable", async () => {
  ...
  const [listing] = await Promise.all([
    collections(session, input),
    collections(session, input),
  ]);
  assert.deepEqual(listing.collections[0], {
    name: '["public","items"]',
    label: '"public"."items"',
    schema: "public",
    table: "items",
...
test("unscoped text search and malformed cursors fail before any data scan", async () => {
  ...
  await assert.rejects(
    documents(session, { ...input, query: "needle" }),
    /Choose a search column/,
  );
  await assert.rejects(
    documents(session, { ...input, after: { id: "1", tenant: { $ne: null } } }),
    /Invalid page cursor/,
  );
  await assert.rejects(
    documents(session, { ...input, page: 500 }),
    /next-page cursor/,
  );
  assert.equal(queries.length, 1);
});
