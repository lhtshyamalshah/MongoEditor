...
test("connecting and listing collections never enumerates databases", async (context) => {
  ...
  context.mock.method(MongoClient.prototype, "db", (databaseName: string) => ({
    ...
    admin: () => {
      adminCalls++;
      throw new Error("Database discovery must not happen");
    },
    listCollections: () => ({
      toArray: async () => [{ name: "items", type: "collection" }],
    }),
  }));
  try {
    ...
    assert.deepEqual(
      await collections(sessionFor(token), { database: "example" }),
...
test("explicit database discovery is name-only, bounded, deduplicated, and cached", async () => {
  ...
  const session = {
    client: {
      db: () => ({
        admin: () => ({
          listDatabases: async (options: Record<string, unknown>) => {
            calls++;
            assert.equal(options.nameOnly, true);
            assert.equal(options.authorizedDatabases, true);
            assert.equal(options.maxTimeMS, 10_000);
