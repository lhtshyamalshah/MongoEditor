...
test("Extended JSON round trips BSON types without precision loss", () => {
  const original = {
    ...
    integer: new BSON.Int32(5),
    double: new BSON.Double(5),
    date: new Date("2026-01-01"),
    binary: new BSON.Binary(Buffer.from([0, 1, 255])),
    nested: { value: null },
  };
  const restored = parseDocument(
    JSON.stringify(wireDocument(original).value),
    "Document",
  );
...
test("document parsing rejects invalid JSON, arrays, scalars and oversize documents", () => {
  for (const value of ["{", "[]", "null", "1", '"text"', " ".repeat(2_000_001)])
    assert.throws(() => parseDocument(value, "Document"), AppError);
});
...
function fixture(
  ...
  const collection = {
    ...
    replaceOne: async (
      ...
      writes.push({ action: "update", filter, replacement, options });
      return { matchedCount: outcome };
    },
    deleteOne: async (filter: Document) => {
      writes.push({ action: "delete", filter });
      return { deletedCount: outcome };
    },
  };
  const db = {
    collection: () => collection,
    listCollections: () => ({ next: async () => ({ name: "items", type }) }),
  };
  const session = {
    client: { db: () => db },
    label: "Test",
  ...
  const input = {
    database: "test",
    collection: "items",
    id: EJSON.stringify({ _id: current?._id ?? "missing" }),
    revision: current ? revision(current) : "missing",
  };
...
test("save preserves ID and uses the entire snapshot in its atomic guard", async () => {
  ...
  await mutate(
    ...
    {
      ...f.input,
      document: EJSON.stringify({ _id: current._id, name: "After" }),
    },
    "update",
  );
...
test("save rejects changes to immutable IDs without writing", async () => {
  ...
  await assert.rejects(
    mutate(f.session, { ...f.input, document: '{"_id":"other"}' }, "update"),
    /_id field/,
  );
...
test("BSON numeric write counters are recognized as successful writes", async () => {
  for (const action of ["update", "delete"] as const) {
    ...
    const result = await mutate(
      f.session,
      { ...f.input, document: JSON.stringify(current) },
      action,
    );
...
test("stale revisions and missing documents cannot be modified or deleted", async () => {
  for (const action of ["update", "delete"] as const) {
    ...
    await assert.rejects(
      mutate(stale.session, { ...stale.input, revision: "stale" }, action),
      (error) => error instanceof AppError && error.status === 409,
    );
    assert.equal(stale.writes.length, 0);
    const missing = fixture(null);
    await assert.rejects(
      mutate(missing.session, missing.input, action),
...
test("deletion uses one exact snapshot; system collections and views cannot be written", async () => {
  ...
  await assert.rejects(
    mutate(
      view.session,
      { ...view.input, collection: "system.users" },
      "delete",
    ),
    /read-only/,
  );
...
test("DocumentDB connections enable TLS and disable retryable writes", () => {
  const options = connectionOptions(
    "mongodb://user:password@cluster.region.docdb.amazonaws.com:27017/db",
  );
...
test("unknown database errors never expose credentials or server details", () => {
  const result = publicError(
    new Error("mongodb://secret:password@private-host/db"),
  );
