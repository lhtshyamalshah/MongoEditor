...
const columns: PgColumn[] = ["id", "name", 'a.b"c'].map((name) => ({
  name,
  type: "text",
  nullable: true,
  primaryKey: name === "id",
...
test("PostgreSQL filters bind values, escape identifiers, and reject unsupported syntax", () => {
  ...
  const result = compileFilter(
    { name: attack, 'a.b"c': { $gte: "10" } },
    columns,
  );
  assert.equal(result.sql, '"name" = $1 AND "a.b""c" >= $2');
  assert.deepEqual(result.values, [attack, "10"]);
  assert.throws(
    () => compileFilter({ "name; DROP TABLE users": "x" }, columns),
    /Unknown column/,
  );
  for (const operator of ["$where", "$regex", "$or", "toString", "__proto__"])
    assert.throws(
      () => compileFilter({ name: { [operator]: "x" } }, columns),
      /Unsupported/,
    );
  assert.throws(
    () => compileFilter({ name: { $eq: { $gt: "x" } } }, columns),
    /Filter values/,
  );
...
test("PostgreSQL text search is literal and null filters use SQL null semantics", () => {
  ...
  assert.equal(
    compileFilter({ name: null, id: { $ne: null } }, columns).sql,
    '"name" IS NULL AND "id" IS NOT NULL',
  );
  assert.throws(
    () => compileFilter({ name: { $gt: null } }, columns),
    /Use \$eq/,
  );
});
test("schema-qualified names preserve dots and quotes and reject system tables", () => {
  assert.equal(
    relationName(JSON.stringify(["a.b", 'c"d'])).sql,
    '"a.b"."c""d"',
  );
  assert.throws(
    () => relationName('["pg_catalog","pg_class"]'),
    /valid user table/,
  );
...
test("database overrides preserve URI credentials and TLS settings", () => {
  const options = poolOptions(
    "postgresql://u:p%40ss@localhost:5432/old?sslmode=verify-full",
    "new db",
  );
...
test("row representation keeps numeric precision and composite identity separate", () => {
  const row = wireRow(
    {
      ...
      date: "2026-01-01 12:34:56.123456",
      json: '{"big":9007199254740993}',
    },
    ["id", "tenant"],
  );
  ...
  assert.notEqual(
    row.revision,
    wireRow({ ...row.value, amount: "0" }, ["id", "tenant"]).revision,
  );
});
test("profiles discover both database engines without exposing connection strings", (context) => {
  for (const [key, value] of Object.entries({
    ...
    POSTGRES_URI: "postgres://user:secret@host/db",
    DB: "mongodb://localhost/db",
    TENANT_DB: "https://not-a-database",
  })) {
