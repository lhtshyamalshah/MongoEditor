...
test("schema detects BSON types, nested fields, mixed types and missing fields", () => {
  const result = inferSchema([
    {
      ...
      amount: BSON.Decimal128.fromString("1.25"),
      large: BSON.Long.fromString("9007199254740993"),
      date: new Date(),
      enabled: true,
  ...
  const field = (path: string) =>
    result.fields.find((item) => item.path === path)!;
...
test("array fields count documents once and retain nested paths and item types", () => {
  const result = inferSchema([
    {
      tags: ["a", "b"],
      orders: [{ total: new BSON.Double(1.5) }, { total: new BSON.Int32(3) }],
    },
    { tags: [] },
  ]);
...
test("empty schemas and traversal limits are explicit", () => {
  ...
  const large = inferSchema([
    Object.fromEntries(
      Array.from({ length: 501 }, (_, index) => [`field${index}`, index]),
    ),
  ]);
  assert.equal(large.fields.length, 500);
  assert.equal(large.limited, true);
  assert.equal(
    inferSchema([{ values: Array.from({ length: 51 }, () => "x") }]).limited,
    true,
  );
...
test("field filters preserve string values and escape regex characters", () => {
  assert.deepEqual(buildFieldFilter("user.name", "eq", "string", " true "), {
    "user.name": { $eq: " true " },
  });
...
test("numeric, ObjectId, date and boolean filters parse into real BSON values", () => {
  const long = parseDocument(
    JSON.stringify(
      buildFieldFilter("total", "gte", "long", "9223372036854775807"),
    ),
    "Filter",
  );
  assert.equal(long.total.$gte.toString(), "9223372036854775807");
  assert.equal(long.total.$gte._bsontype, "Long");
  const id = parseDocument(
    JSON.stringify(
      buildFieldFilter("_id", "eq", "objectId", "507f1f77bcf86cd799439011"),
    ),
    "Filter",
  );
  assert.equal(id._id.$eq._bsontype, "ObjectId");
  const date = parseDocument(
    JSON.stringify(buildFieldFilter("createdAt", "lt", "date", "2026-09-11")),
    "Filter",
  );
  assert.equal(date.createdAt.$lt.toISOString(), "2026-09-11T00:00:00.000Z");
  assert.deepEqual(buildFieldFilter("enabled", "eq", "boolean", "false"), {
    enabled: { $eq: false },
  });
});
...
test("exists, missing and explicit null have distinct filters", () => {
  assert.deepEqual(buildFieldFilter("optional", "exists", "date", ""), {
    optional: { $exists: true },
  });
  assert.deepEqual(buildFieldFilter("optional", "missing", "date", ""), {
    optional: { $exists: false },
  });
  assert.deepEqual(buildFieldFilter("optional", "eq", "null", ""), {
    optional: { $eq: null, $exists: true },
  });
});

test("object values are equality operands, never injected query operators", () => {
  assert.deepEqual(buildFieldFilter("payload", "eq", "json", '{"$ne":null}'), {
    payload: { $eq: { $ne: null } },
  });
...
test("schema reads an unfiltered bounded collection sample", async () => {
  ...
  const cursor = {
    ...
    toArray: async () => sample,
  };
  const collection = {
    find: (filter: object, options: { maxTimeMS: number }) => {
      assert.deepEqual(filter, {});
      assert.ok(options.maxTimeMS > 0);
      return cursor;
    },
  };
  const session = {
  ...
  const result = await schema(session, {
    database: "db",
    collection: "items",
    query: "should not affect schema",
  });
