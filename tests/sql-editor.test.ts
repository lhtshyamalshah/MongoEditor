...
import { prepareReadQuery, referencedTables, sqlParameters } from "../lib/sql-editor";
...
test("parameters ignore quoted values, identifiers, dollar strings and nested comments", () => {
  ...
  assert.throws(() => sqlParameters("SELECT $999999999999999"), /\$1 through \$50/);
});
test("read query validation accepts SELECT and read CTEs, preserves binds and bounds results", () => {
  const prepared = prepareReadQuery("/* header */ WITH x AS (SELECT $1::text AS value) SELECT * FROM x; -- trailing", ["';DELETE FROM x;--"], 25);
...
test("SQL validation rejects scripts, write CTEs, wrapper escapes and invalid parameters", () => {
  for (const sql of ["DELETE FROM x", "UPDATE x SET a=1", "WITH x AS (DELETE FROM y RETURNING *) SELECT * FROM x", "SELECT 1; DELETE FROM x", "SELECT 1; SELECT 2", "SELECT * INTO tmp FROM x", "SELECT * FROM x FOR UPDATE", "SELECT 1) AS x", "SELECT (1", "SELECT 'unclosed", "SELECT $$unclosed", "SELECT 1 /* unclosed"]) assert.throws(() => prepareReadQuery(sql, [], 100));
  assert.throws(() => prepareReadQuery("SELECT $2::text", ["x"], 100), /consecutive/);
  assert.throws(() => prepareReadQuery("SELECT $1", [], 100), /one text value/);
  assert.throws(() => prepareReadQuery("SELECT $1", [{ value: "x" }], 100), /one text value/);
  assert.throws(() => prepareReadQuery("SELECT 1", [], 10_000), /result limit/);
  assert.throws(() => prepareReadQuery("SELECT " + " ".repeat(50_000), [], 100), /50,000/);
});
test("column hints load only referenced known tables and recognize quoted names", () => {
  ...
  assert.deepEqual(referencedTables('SELECT u.id FROM public.users u JOIN "sales"."order.items""quoted" o ON o.id=u.id -- FROM ignored', tables), ["users", "orders"]);
  assert.deepEqual(referencedTables("SELECT 'FROM ignored' FROM users", tables), ["users"]);
});
