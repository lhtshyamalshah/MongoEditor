...
const tables = [
  ...
  { name: "odd", schema: "a.b", table: 'odd.table"name' },
];
const namespace = completionNamespace(tables, {
  users: {
    columns: [
      {
        name: "id",
...
async function suggestions(document: string, pos = document.length) {
  ...
  return (
    (
      await schemaCompletionSource(config)(
