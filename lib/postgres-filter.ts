...
export function compileFilter(
  ...
  for (const [field, condition] of Object.entries(filter)) {
    ...
    if (!Object.keys(operators).length)
      throw new Error("A filter must contain an operator.");
    for (const [op, value] of Object.entries(operators)) {
      if (
        ...
        throw new Error(
          "Filter values must be strings, numbers, booleans, or null. Use strings for SQL arrays and JSON.",
        );
      if (value === null) {
        if (!["$eq", "$ne"].includes(op))
          throw new Error("Use $eq or $ne with null.");
        clauses.push(`${name} IS ${op === "$ne" ? "NOT " : ""}NULL`);
      } else if (op === "$contains") {
        if (typeof value !== "string")
          throw new Error("$contains requires text.");
        clauses.push(
          `${name}::text ILIKE ${bind(`%${value.replace(/[\\%_]/g, "\\$&")}%`)} ESCAPE E'\\\\'`,
        );
      } else {
        ...
        if (!Object.hasOwn(allowed, op))
          throw new Error(`Unsupported PostgreSQL filter operator: ${op}`);
