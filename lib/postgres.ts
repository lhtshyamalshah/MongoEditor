...
async function cachedMetadata<T>(
  ...
  if (
    existing &&
    (existing.pending || (!refresh && existing.expires > Date.now()))
  )
  ...
  try {
    ...
    entry.pending = false;
    entry.expires = Date.now() + METADATA_TTL;
    return value;
  } catch (error) {
...
const globalStore = globalThis as typeof globalThis & {
  postgresBrowserSessions?: Map<string, Session>;
  postgresBrowserTimer?: ReturnType<typeof setInterval>;
};
const sessions = (globalStore.postgresBrowserSessions ??= new Map<
  string,
  Session
>());
if (!globalStore.postgresBrowserTimer) {
  globalStore.postgresBrowserTimer = setInterval(() => {
    for (const [token, session] of sessions)
      if (Date.now() - session.touched > TTL) void disconnect(token);
  }, 60_000);
...
export function hasSession(token: string | null) {
  return !!token && sessions.has(token);
}
export function sessionFor(token: string | null) {
  const session = token ? sessions.get(token) : undefined;
  if (!session || Date.now() - session.touched > TTL)
    throw new AppError(
...
export function poolOptions(uri: string, database?: string) {
  ...
  try {
    ...
    throw new AppError("Enter a valid PostgreSQL connection string.");
  }
  if (!["postgres:", "postgresql:"].includes(url.protocol))
    throw new AppError(
      "Enter a postgres:// or postgresql:// connection string.",
    );
  if (database) {
    url.pathname = `/${encodeURIComponent(name(database, "database"))}`;
  ...
  return {
    ...
    application_name: "local-database-browser",
  };
}
...
async function poolFor(session: Session, database: unknown) {
  ...
  if (session.pools.size >= 5)
    throw new AppError(
      "Five databases are already open. Reconnect to open another database.",
    );
...
export async function connect(input: Input) {
  if (sessions.size >= 20)
    throw new AppError(
      "Too many open connections. Disconnect an existing connection first.",
      429,
    );
  ...
  try {
    ...
    return {
      ...
      label: input.profile || "Custom connection",
      notice: "",
    };
  } catch (error) {
    await pool.end().catch(() => {});
...
export async function databases(session: Session) {
  session.databaseNames ??= poolFor(session, session.database)
    .then((pool) =>
      pool.query(
        "SELECT datname FROM pg_catalog.pg_database WHERE datallowconn AND NOT datistemplate AND has_database_privilege(oid, 'CONNECT') ORDER BY datname",
      ),
    )
    .then((result) => result.rows.map((row) => row.datname as string))
    .catch((error) => {
...
export function relationName(value: unknown) {
  try {
    ...
    const schema = name(pair[0], "schema"),
      table = name(pair[1], "table");
    if (schema === "information_schema" || schema.startsWith("pg_"))
      throw new Error();
    return { schema, table, sql: `${qi(schema)}.${qi(table)}` };
  } catch {
    throw new AppError("Choose a valid user table.");
  }
}
export async function collections(session: Session, input: Input) {
  ...
  return cachedMetadata(
    session,
    JSON.stringify([input.database, "tables"]),
    async () => {
      const result =
        await pool.query(`SELECT n.nspname, c.relname, c.relkind, pg_catalog.pg_get_userbyid(c.relowner) AS owner,
      ...
      return {
        collections: result.rows.map((row) => ({
          name: JSON.stringify([row.nspname, row.relname]),
          label: `${qi(row.nspname)}.${qi(row.relname)}`,
          schema: row.nspname as string,
          table: row.relname as string,
          owner: row.owner as string,
          type: ["v", "m"].includes(row.relkind) ? "view" : "table",
          readOnly: !row.pk || !["r", "p"].includes(row.relkind),
        })),
      };
    },
    input.refresh === true,
...
async function tableInfo(client: Pool | PoolClient, collection: unknown) {
  ...
  const result = await client.query(
    ...
    WHERE n.nspname = $1 AND c.relname = $2 AND c.relkind IN ('r', 'p', 'v', 'm') AND a.attnum > 0 AND NOT a.attisdropped ORDER BY a.attnum`,
    [relation.schema, relation.table],
  );
  if (!result.rows.length)
    throw new AppError("This table no longer exists or has no columns.", 404);
  const columns = result.rows.map(
    ({ name, type, nullable, generated, keyPosition }) => ({
      name,
      type,
      nullable,
      generated,
      primaryKey: keyPosition !== null,
    }),
  ) as PgColumn[];
  // Match the primary-key index order, which can differ from column order.
  const primaryKeys = result.rows
    .filter((column) => column.keyPosition !== null)
    .sort((a, b) => a.keyPosition - b.keyPosition)
    .map((column) => column.name as string);
  const readOnly =
    !["r", "p"].includes(result.rows[0].relkind) || !primaryKeys.length;
  return {
    ...relation,
    columns,
    primaryKeys,
...
function readTableInfo(session: Session, pool: Pool, input: Input) {
  return cachedMetadata(
    session,
    JSON.stringify([input.database, "schema", input.collection]),
    () => tableInfo(pool, input.collection),
    input.refresh === true,
  );
}
export async function schema(session: Session, input: Input) {
  const { columns, readOnly, reason } = await readTableInfo(
    session,
    await poolFor(session, input.database),
    input,
  );
...
export async function query(session: Session, input: Input) {
  ...
  if (session.queryRunning)
    throw new AppError(
      "A SQL query is already running for this connection.",
      429,
    );
  ...
  try {
    ...
    return {
      ...
      rows,
      truncated,
      rowLimit: prepared.limit,
      durationMs: Date.now() - started,
      notice:
        bytes > 2_000_000
          ? "Results exceed 2 MB. Select fewer or smaller columns."
          : truncated
            ? `Showing the first ${prepared.limit} rows. Narrow the query to see other results.`
            : "",
    };
  } catch (error) {
    ...
    if (code === "25006")
      throw new AppError(
        "The SQL editor is read-only. This query attempted to change data.",
        403,
      );
    if (code === "42601")
      throw new AppError(
        "SQL syntax error. Check the query and run one SELECT or WITH statement.",
      );
    if (code === "42P01")
      throw new AppError(
        "A table or view was not found. Use its schema-qualified name from autocomplete.",
      );
    if (code === "42703")
      throw new AppError(
        "A column was not found. Check the column suggestions and table aliases.",
      );
    if (code === "42P18" || code === "42804")
      throw new AppError(
        "PostgreSQL could not infer a parameter type. Add a cast such as $1::uuid or $1::text.",
      );
    throw error;
  } finally {
    if (client) {
...
function projection(columns: PgColumn[]) {
  return columns
    .map((column) => `${qi(column.name)}::text AS ${qi(column.name)}`)
    .join(", ");
}
export function wireRow(value: Row, keys: string[]) {
  return {
    value,
    id: Object.fromEntries(keys.map((key) => [key, value[key]])),
...
export async function documents(session: Session, input: Input) {
  ...
  try {
    if (input.mode === "json") {
      if (typeof input.query !== "string" || input.query.length > 2_000_000)
        throw new Error("Enter a JSON filter under 2 MB.");
      filter = compileFilter(
        object(JSON.parse(input.query || "{}"), "Filter"),
        table.columns,
      );
    } else if (typeof input.query === "string" && input.query.trim()) {
      if (input.query.length > 500)
        throw new Error("Text searches must be 500 characters or fewer.");
      if (
        typeof input.field !== "string" ||
        !table.columns.some((column) => column.name === input.field)
      )
        throw new Error(
          "Choose a search column to avoid searching every column. Indexed JSON filters are preferable on large tables.",
        );
      filter = compileFilter(
        { [input.field]: { $contains: input.query.trim() } },
        table.columns,
      );
    }
  } catch (error) {
    throw new AppError(
      error instanceof SyntaxError
        ? "Enter a valid JSON filter."
        : (error as Error).message,
    );
  }
  const keyed = table.primaryKeys.length > 0;
  if (!keyed && (page !== 1 || input.after != null))
    throw new AppError(
      "Tables without a primary key show one limited preview. Apply a filter to narrow the results.",
    );
  if (keyed && page > 1 && input.after == null)
    throw new AppError(
      "Use the next-page cursor to continue browsing. Refresh to restart.",
    );
  if (input.after != null) {
    const after = object(input.after, "Page cursor");
    if (
      Object.keys(after).length !== table.primaryKeys.length ||
      table.primaryKeys.some(
        (key) => !Object.hasOwn(after, key) || typeof after[key] !== "string",
      )
    )
      throw new AppError("Invalid page cursor. Refresh to restart.");
    const parameters = table.primaryKeys.map((key) => {
      filter.values.push(after[key]);
      return `$${filter.values.length}`;
    });
    filter.sql = `(${filter.sql}) AND (${table.primaryKeys.map((key) => `data.${qi(key)}`).join(", ")}) > (${parameters.join(", ")})`;
  }
  const order = keyed
    ? ` ORDER BY ${table.primaryKeys.map((key) => `data.${qi(key)}`).join(", ")}`
    : "";
  ...
  try {
    result = await pool.query(
      `SELECT ${projection(table.columns)} FROM ${table.sql} AS data WHERE ${filter.sql}${order} LIMIT $${filter.values.length + 1}`,
      [...filter.values, keyed ? pageSize + 1 : pageSize],
    );
  } catch (error) {
  ...
  const rows = result.rows
    .slice(0, pageSize)
    .map((row) => wireRow(row, table.primaryKeys));
  ...
  return {
    ...
    pagination: keyed ? "keyset" : "preview",
    notice: keyed
      ? ""
      : "Limited preview: this table or view has no primary key. Apply a filter to narrow the results. Rows are not sorted.",
...
export async function mutate(
  ...
  try {
    ...
    if (JSON.stringify(verified) !== JSON.stringify(table))
      throw new AppError(
        "The table schema changed. Refresh and try again.",
        409,
      );
    const id = parseRow(input.id, "Row key");
    if (
      Object.keys(id).length !== table.primaryKeys.length ||
      table.primaryKeys.some(
        (key) => !Object.hasOwn(id, key) || typeof id[key] !== "string",
      )
    )
      throw new AppError("The complete primary key is required.");
    const values = table.primaryKeys.map((key) => id[key]);
    const where = table.primaryKeys
      .map((key, index) => `${qi(key)} = $${index + 1}`)
      .join(" AND ");
    const found = await client.query(
      `SELECT ${projection(table.columns)} FROM ${table.sql} WHERE ${where} FOR UPDATE`,
      values,
    );
    if (!found.rows.length)
      throw new AppError("This row no longer exists. Refresh the table.", 404);
    if (found.rows.length !== 1)
      throw new AppError("The key does not identify exactly one row.", 409);
    const current = found.rows[0] as Row;
    if (wireRow(current, table.primaryKeys).revision !== input.revision)
      throw new AppError(
        "This row changed after you opened it. Refresh and reopen it before making changes.",
        409,
      );
    if (action === "delete") {
      const result = await client.query(
        `DELETE FROM ${table.sql} WHERE ${where}`,
        values,
      );
      if (result.rowCount !== 1)
        throw new AppError("The row could not be deleted.", 409);
    } else {
      const next = parseRow(input.document, "Row");
      if (
        Object.keys(next).length !== table.columns.length ||
        table.columns.some((column) => !Object.hasOwn(next, column.name))
      )
        throw new AppError(
          "Keep every column in the row. Use null for SQL NULL.",
        );
      const changed = table.columns.filter(
        (column) => next[column.name] !== current[column.name],
      );
      for (const column of changed) {
        if (column.primaryKey || column.generated)
          throw new AppError(
            "Primary key and generated columns cannot be changed.",
          );
        if (next[column.name] !== null && typeof next[column.name] !== "string")
          throw new AppError(
            "Use strings for PostgreSQL values, or null for SQL NULL.",
          );
      }
      if (changed.length) {
        const set = changed
          .map((column) => {
            values.push(next[column.name]);
            return `${qi(column.name)} = $${values.length}`;
          })
          .join(", ");
        const result = await client.query(
          `UPDATE ${table.sql} SET ${set} WHERE ${where}`,
          values,
        );
        if (result.rowCount !== 1)
          throw new AppError("The row could not be updated.", 409);
      }
    }
...
export function publicError(error: unknown) {
  ...
  if (code === "28P01" || code === "28000")
    return {
      message:
        "PostgreSQL authentication failed. Check your username and password.",
      status: 401,
    };
  if (code === "42501")
    return {
  ...
  if (code === "3D000")
    return {
      message: "The requested PostgreSQL database does not exist.",
      status: 404,
    };
  if (code === "23505")
    return { message: "A unique column already has this value.", status: 409 };
  if (code === "23503")
    return {
      message: "This change conflicts with a foreign key constraint.",
      status: 409,
    };
  if (code === "57014" || code === "55P03")
    return {
      message:
        "The query or lock timed out. Try a more specific filter or retry later.",
      status: 408,
    };
  if (code === "40001" || code === "40P01")
    return {
      message:
        "A concurrent transaction conflicted with this change. Refresh and try again.",
      status: 409,
    };
  if (code && /^(22|23|42)/.test(code))
    return {
      message:
        "PostgreSQL rejected the value or filter. Check column types, operators, and table constraints.",
      status: 400,
    };
  if (
    ...
    return {
      message:
        "Cannot reach PostgreSQL. Check the host, port, VPN, and connection settings.",
      status: 503,
    };
  return null;
}
